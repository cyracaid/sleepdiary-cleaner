...
After all checkpoints, `final_summary()` compares the first meaningful checkpoint (B — first with data_category) to the last (E), printing a delta table and saving to `output/correction_status_final.csv`.
...
| `tests/testthat/test-interval.R` | Malformed colon formats ("00:000" → "00:00", "000:45" → "00:45") |
