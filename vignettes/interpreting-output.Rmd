...
`run_pipeline()` 跑完后，`output/correction_status_final.csv`（运行摘要）和
...
`data_category`（Step 6 起冻结，`equal_time_ok + skipped_na = n_total`）、
`flag_severity`（Step 7 起冻结，Clean+Minor+Major = n_total − skipped_na）、
`duration_extreme`（Too short + Too long < 5%）、`checkforerrors`（仅 Step 8）。

**回归检查**：重跑前先 `file.copy` 存基线，重跑后 `identical()` 对比关键列。
输入未变但结果不同 → 管线输出改变了，需调查。

**快速检查卡**：管线完成（`correction_status_final.csv` 存在）、TST 6–8.5、
SOL 10–45、n_error < 1%、各标准计数冻结、全记录有交代、确定性。

**图表**：`latest_visualization_<tag>_n<rows>/` 每次运行覆盖，`figure_index.png`
总览。出版用图：Figure 1 管线流程（各阶段计数）、Figure 2 校正影响（delta
lollipop + 散点）。5 张必看图：质量仪表板、校正进度（C 检查点后才出现
Corrected 条）、自动检测仪表板、睡眠变量分布、统一质量状态（Error+Unusual < 5%）。
