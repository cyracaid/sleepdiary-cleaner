---
title: "如何读懂管线输出（中文）"
description: "怎么读 correction_status_final.csv、step flag ledger、回归检查与诊断图表——每个输出的含义和检查要点。"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{如何读懂管线输出（中文）}
...
`run_pipeline()` 跑完后，两个 CSV 文件告诉你一切。本文说明怎么读它们、怎么
对比上一次运行做回归检查、以及怎么看图。
...
1. `field_misentry` — Step 1.5 起有数。任何 `SOL=time_bed` 或 `WASO=time_getup`
   `count > 0` = 潜在跨字段污染。
2. `data_category` — Step 6 起数字必须**稳定**：`equal_time_ok + skipped_na = n_total`。
...
`output/correction_status_old.csv` 不是任何管线脚本写的——重跑前你自己另存一份
基线：
...
`figure_index.png` 总览所有图。稳定验证产物（snapshot、Bland-Altman 图、阈值
验证）单独在 `output/verification/<tag>/`。
