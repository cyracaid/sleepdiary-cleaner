...
- [`guess_column_mapping()`](https://cyracaid.github.io/sleepdiary-cleaner/reference/guess_column_mapping.md)
  returns per-column records (user_col, internal_col, match_rule,
  confidence, status, candidate_set); ambiguous ties are never silently
  resolved; absent required fields fail loudly.
- `dry_run = TRUE` previews the mapping and writes only
  `dry_run_manifest.json` (never a cleaned dataset); raw input is never
  modified.
...
- [`run_pipeline()`](https://cyracaid.github.io/sleepdiary-cleaner/reference/run_pipeline.md)
  gains an optional `data = NULL` argument; default NULL is the
  zero-change backward-compatible path (invariant-tested).
...
- `validation/synthetic/benchmark_table.R`: per-category detection-vs-
  ...
  fixes the control-row miscompute in detection_outcomes_v4_current.csv.
