...
a different way, and is deliberately independent of the others.
      ...
      ────────────────────────────────────────────────────────────
      Step 1: Clean-input specificity ─── records altered / false alarms
      ...
      Step 2: Injected-error benchmark ── pooled recall 0.995 [0.993, 0.997]
               (400 errors/category; ground truth recorded at injection)
                         │
                         ▼
      Step 3: Detection vs value-correctness ── flagged vs actually right gap
               (e.g. ampm_swap flagged 1.0 / value-correct 0.565 → routed to human)
                         │
                         ▼
      ...
      ────────────────────────────────────────────────────────────
      Step 8: Multiverse + downstream + seeds ── full grid: adjacent-swap threshold = 44%
               (seed-fragile → main curve AM/PM-flip only, full factorial in
...
corrected** (detection level) and **value-correct** (final value
actually equals the ground truth). The gap between them is where human
review lives.

**Result.** `ampm_swap`: flagged 1.0, value-correct 0.565.
`field_misentry`: flagged 1.0, value-correct 0.018. **Mis-repair rate**
in the current run: **0 across all categories**.
...
of guessing — that is the design working. The `field_misentry`
...
|-----|------------------------------|-----------------------------------------------------------------------|-------------------------------------------------------------------------------|
| A1  | **SOL Bland-Altman**         | self-reported SOL vs computed SOL                                     | bias, **95% limits of agreement**, half-width, % outside, proportional-bias p |
| A2  | **WASO Bland-Altman**        | self-reported WASO vs computed WASO                                   | same as A1                                                                    |
| A3  | **Threshold vs noise ratio** | each flag threshold ÷ its metric’s **limits of agreement half-width** | per-threshold verdict: CONSERVATIVE / SAFE / BORDERLINE / INSIDE NOISE        |

**Result.**

|         | Bias     | **Limits of agreement half-width** | Threshold            | Ratio | Verdict                   |
|---------|----------|------------------------------------|----------------------|-------|---------------------------|
...
5.  **Limits of agreement bound *consistency*, never *accuracy*** — do
    not cite them as criterion validity (that would require actigraphy /
...
1.  **Multiverse analysis (specification curve).** Define the cleaning
    ...
    midnight 6, mmss 60, SOL 120/180, SE 70, WASO 1.5, TST/TIB 0.5,
    cross-participant spike cutoff 4× **median absolute deviation
    (MAD)** — plus discrete branches: which rows to trust,
    cross-participant layer on/off, manual-review layer on/off). Hard
    constraints are excluded (those are logic, not choice). Screen with
    **one-at-a-time** runs, then a full factorial grid; each spec runs
    the real pipeline and records the same outputs. Two output
    contracts: a **specification curve** (Simonsohn-style plot of the
    metric across all specs) and a **variance decomposition** (how much
    of the variation each dimension explains). Script:
...
|-------------------------|------------------------|
| Adjacent-swap threshold | **44%** (seed-fragile) |
| AM/PM flip trigger      | 10%                    |
| others                  | remainder              |

The adjacent-swap threshold’s 44% holds only inside the full factorial
grid and is **seed-fragile**: one-at-a-time screening keeps AM/PM flip +
adjacent-swap under seed 20260817 but reduces the surviving set to AM/PM
flip only under seed 20260915. The *main* specification curve is
therefore reported **AM/PM-flip only** (3 specs — the cross-seed
intersection), and the full factorial lives in the appendix as
`spec_curve_full.csv`.
...
*Decision-relevant deltas (branch 1 / branch 2):* branch 1 (which rows
to trust) moves mean TST by **29.6 min**; branch 2 (WASO trust gate
variant) moves analyzable n by **1,131 records**. These are not errors —
they are the honest range of what a *different reasonable* decision
produces.

*Seeds:* pooled recall stable at 0.993–0.995 across 4 seeds, control
**false alarm rate 0** in all; the one family that wobbles (0.886–0.907)
...
decision matters most?* — the adjacent-swap threshold dominates the full
grid at 44% of variation, but that share is seed-fragile, so the main
curve is AM/PM-flip only and the adjacent-swap threshold is a documented
sensitivity warning; (2) *which conclusions are robust?* — TST yes
(±0.01 h on the AM/PM-flip main curve), SOL no (23–58 min), and we now
know why (perception bias, Step 5.5); (3) *are the numbers
reproducible?* — yes across seeds (recall 0.993–0.995, **false alarm
...
1.  **`cross_participant_spike` is the weakest family** — flagged
    0.886–0.907 across seeds, value-correct 0 *by design* (it is
    audit-only: a single-day spike may be a real event — illness, shift
    work — so it flags for a human and never auto-fixes).
...
3.  **Ablation recall uses a flag-based definition** (auto-corrected
    records never enter the flag queue, so disabling a fixing rule
