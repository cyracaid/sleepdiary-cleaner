...
|------------------------------|------------------------|
| D2 (adjacent-swap threshold) | **44%** (seed-fragile) |
| D1 (AM/PM flip trigger)      | 10%                    |
| others                       | remainder              |

D2’s 44% holds only inside the full factorial grid and is
**seed-fragile**: one-at-a-time (OAT) screening keeps D1 + D2 under seed
20260817 but reduces the surviving set to D1 only under seed 20260915.
The *main* specification curve is therefore reported **D1-only** (3
specs — the cross-seed intersection), and the full factorial lives in
the appendix as `spec_curve_full.csv`.
...
decision matters most?* — the swap threshold dominates the full grid at
44% of variation, but that share is seed-fragile, so the main curve is
D1-only and D2 is a documented sensitivity warning; (2) *which
conclusions are robust?* — TST yes (±0.01 h on the D1-only main curve),
SOL no (23–58 min), and we now know why (perception bias, Step 5.5); (3)
*are the numbers reproducible?* — yes across seeds (recall 0.993–0.995,
