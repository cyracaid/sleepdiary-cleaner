...
The retired figures `08_Sleep_Duration_by_Category` and
`12_Pipeline_Correction_Progress` are no longer produced (their
...
`sleep_visualization.R:832` **Paper use** Methods Fig 1 **What each part
means** Vertical flow of 5 stages: Raw Load → Parsed → Algorithmic
Correction → Manual Correction → Final Valid. Each box = record count +
% of total; two correction stages also show participant counts.
**Right-side annotation** breaks the final classification: Not Reported,
...
means** 4 panels: (A) TST delta lollipops — one row per modified record,
x = ΔTST min, color orange = algorithmic, blue = manual; (B) same for
SOL; (C) identity scatter TST-before vs TST-after, unchanged records at
3% opacity as gray backdrop, diagonal reference line; (D) summary table
of TST/SOL mean ± SD before vs after. Caption states how many of how
many records were modified. **Healthy look** Only a small fraction of
points deviate from the diagonal; deltas clustered near 0; before/after
means nearly identical (non-destructive cleaning). **Anomaly →** Huge
deltas or many rows → corrections are changing real data, not just
...
means** Violin plots + inner boxplots, one per sleep variable, **free Y
axis per panel**. Shows distribution shape and spread. **Healthy look**
Roughly symmetric violins for TST; right-skewed SOL/WASO; no panel
dominated by spikes. **Anomaly →** A violin split into two lobes →
...
each part means** Two density curves over hour-of-day (0–24): bedtime
distribution vs get-up distribution (final corrected times). **Healthy
look** Bedtime peak 22:00–00:00; get-up peak 06:00–08:00; curves don’t
overlap much. **Anomaly →** Bedtime peak after 02:00 → delayed-sleep
...
(60–95%); long-sleep points high SE. **Anomaly →** Cluster of
short-sleep points below SE 85% → genuine insomnia vs measurement issue;
...
prefixes (Temporal / Metrics / Amount / Interval / Timestamp). **Healthy
look** One or two dominant categories (usually Temporal); all categories
small in absolute terms. **Anomaly →** Timestamp or Interval format
...
part means** Histogram of gap hours between adjacent diary events
(bed→sleep, sleep→awake, awake→getup), **pre-correction raw times**.
Negative gap = the later event’s clock time precedes the earlier one.
Vertical reference line at 0. **Healthy look** Mass of gaps in +0.5 to
+2 h (positive, sane order); a small negative tail. **Anomaly →** Large
...
**What each part means** Tile heatmap from synthetic error-injection
validation: rows/columns = injected error types, cells = detection
outcome counts vs modal result. Validates the detector fires on the
errors it should. **Healthy look** Strong diagonal (detected where
injected); off-diagonal near zero. **Anomaly →** Off-diagonal mass →
...
**What each part means** Columns showing detection rate against the
**threshold-to-Bland-Altman-noise ratio**; vertical line = the chosen
operating point. Ties cleaning thresholds to measurement noise rather
than arbitrary cutoffs. **Healthy look** Detection rate high and flat to
the right of the vertical line; no cliff at the chosen point. **Anomaly
...
`correction_status_final.csv`. **Anomaly →** Counts disagree with the
...
and stay constant after; `equal_time_ok + skipped_na = n_total` from
...
**What each part means** Bar chart of participants with the highest
flagged-record rate, for manual review prioritization. **Healthy look**
Short list of a few participants; no runaway outlier. **Anomaly →** Long
...
each part means** Corrplot upper-triangle of pairwise Pearson
correlations among TST, SOL, WASO, SE, TIB (final corrected data). Red =
negative, green = positive, coefficient printed in each cell. **Healthy
look** Strong expected signs: TST–SE strongly positive, SOL–SE negative,
TST–TIB positive, WASO–SE negative. **Anomaly →** Sign flips
...
**`equal_time_ok` (Figure 01, `data_category`).** Counts records where
two adjacent diary timestamps came out identical but the record is
otherwise valid. Fires when bedtime == sleep time (zero sleep latency)
**and/or** awake time == getup time (no lingering), **and** the full
...
zero-length sleep period) is **never** classified as Equal Time — that
is always an error, since a night with zero total sleep duration is a
broken record, not a benign reporting pattern. Equal Time is
specifically the harmless case where a participant reported two adjacent
checkpoints (e.g. “went to bed” and “fell asleep”) as the same clock
time — a common, legitimate way to fill a sleep diary. Such records flow
straight through as valid rather than going to manual review.

**`skipped_na`.** Any record with one or more of the four corrected
timestamps missing is classified `skipped_na`, and is excluded from
temporal-order evaluation. From Step 6 onward
`equal_time_ok + skipped_na = n_total` must hold — this identity is the
