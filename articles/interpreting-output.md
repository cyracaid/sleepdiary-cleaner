...
-   **fill = `flag_severity`**, not `data_category`: `Clean` (0 metric
    ...
    are the natural home of Major flags.
...
-   Two densities over hour-of-day \[0, 24): bedtime
    ...
    is the healthy circadian signature.
...
pipeline’s physiological limits (`duration_extreme`: &lt; 3 h / &gt; 12
h). Don’t confuse the two sets of thresholds.
...
-   The distribution tells you which failure mode dominates: a negative
    ...
    by the 12 h flip, `flip_gap_hours = 12`).
...
-   Extreme values feed `AMOUNT_FLAG` in Step 8; a spike at an
    ...
    the source export (cups vs cans vs servings).
...
-   `day_type` is derived from the corrected bedtime:
    ...
    jetlag worth reporting as a finding.
...
-   **Identity:** `TIB = TST + SOL + WASO` — the figure stacks these
    ...
    of TIB, SOL and WASO small slices.
...
&lt; 70%, SOL &gt; 1 h, WASO &gt; 1.5 h}). Shows whether quality
problems concentrate at extreme durations.
