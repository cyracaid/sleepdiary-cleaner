...
test_that("00_MAIN_entry auto-runs the full legacy chain on synthetic data", {
  ...
  writeLines(c(
    ...
    "  probe <- up",
    "}",
    "# load_all only when a real source tree is reachable (.Rbuildignore + R/",
    "# present): under R CMD check the test runs from .../Rcheck/tests/testthat",
    "# (no .Rbuildignore), and under covr the tests run against the installed",
    "# build-tree copy (an installed DESCRIPTION at the probe path, but no",
    "# .Rbuildignore -- covr instruments the installed package). In both cases",
    "# library(sleepcleanr) is the correct code and load_all would probe a",
    "# misleading DESCRIPTION.",
    "if (nzchar(root_pkg) && file.exists(file.path(root_pkg, '.Rbuildignore')) &&",
