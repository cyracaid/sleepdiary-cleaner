...
#
# Key difference from test-pipeline.R: we must NOT set splsleep_loaded in the
...
test_that("00_MAIN_entry auto-runs the full legacy chain on synthetic data", {
  ...
  # sdir <- get0("splsleep_scripts_dir", .GlobalEnv, ifnotfound = getwd())
  ...
  # user - setwd(sandbox), pre-wire splsleep_scripts_dir + a local config
  # override file, then source the script and expect the auto-run branch.
  #
  # The legacy chain expects splsleep_loaded ABSENT (auto-run) but scripts_dir
  # PRESENT (it locates step scripts). We satisfy both: splsleep_scripts_dir is
  # set by the runner, splsleep_loaded is never created.
  ...
  writeLines(c(
    ...
    "# and the installed package (library(splsleep)) is the correct code anyway.",
    ...
    "options(warn = 1)",
    "time0 <- Sys.time()",
    "res <- tryCatch(source(file.path(get0('splsleep_scripts_dir'), '00_MAIN_entry.R'), local = FALSE),",
    "                 error = function(e) e)",
