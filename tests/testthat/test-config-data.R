...
test_that("main: RDS without extra works", {
  ...
  result <- sleepcleanr:::load_config(NULL)
  # Verify key resolution via .resolve_data_key
  main_val <- sleepcleanr:::.resolve_data_key(cfg, "data.files.main")
  extra_val <- sleepcleanr:::.resolve_data_key(cfg, "data.files.extra")
...
test_that("column mapping transforms user columns to internal names", {
  ...
  result <- sleepcleanr::adapt_columns(df, cfg)
