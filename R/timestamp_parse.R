#' Internalised timestamp parser (was process_timestamp_emadatarelease_cyra.R)
#'
#' v1.3.0 wrapper-first migration internalised the body of the legacy
#' \code{inst/scripts/process_timestamp_emadatarelease_cyra.R} into the
#' package (TECH_DEBT item 2). The body is copied verbatim from the script --
#' the script still ships in \code{inst/scripts/} and is still sourced by
#' \code{00_MAIN_entry.R} and the snapshot verifier, so the two copies must
#' stay logically identical.
#'
#' @param df A data frame.
#' @param varname Character. Base name of the timestamp variable
#'   (e.g. \code{time_bed_am}).
#' @param format Character. Only \code{"timestamp"} is used.
#' @return The data frame with added \code{*_hhmm_ampm} and
#'   \code{*_checkforerrors} columns.
#' @keywords internal
#' @noRd
#' @importFrom dplyr mutate mutate_at case_when one_of vars sym %>%
#' @importFrom tidyr separate
#' @importFrom lubridate hour as_datetime parse_date_time days
