#' Internalised interval parser (was process_interval.R)
#'
#' v1.3.0 wrapper-first migration internalised the body of the legacy
#' \code{inst/scripts/process_interval.R} into the package (TECH_DEBT item 2).
#' The body is copied verbatim from the script -- the script still ships in
#' \code{inst/scripts/} and is still sourced by \code{00_MAIN_entry.R}, the
#' snapshot verifier and \code{test-interval.R}, so the two copies must stay
#' logically identical.
#'
#' @param df A data frame.
#' @param varname Character. Base name of the interval variable
#'   (e.g. \code{duration_totalmin_sol_estimate_am}).
#' @param format Character. Only \code{"interval_hhmm"} is used.
#' @return The data frame with added \code{*_mincalc}, \code{*_checkforerrors}
#'   and \code{*_correctionsmade} columns; the original \code{varname} column
#'   is replaced with the cleaned-up interval strings.
#' @keywords internal
#' @noRd
