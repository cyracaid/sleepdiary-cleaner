# Internalised normalise_sleep_time_sequence (was normalize_sleep_time_sequence.R)
#
...
normalize_sleep_time_sequence <- function(AM_rawdata, flip_gap_hours = 12) {
  ...
  if (sum(!cleaned_data$has_na) > 0) { # Count number of records without NA values
    ...
    for (i in valid_indices) { # Loop framework for row-by-row processing
      ...
      if (getup > awake && awake_getup_diff >= flip_gap_hours) { 
        # Loop adjustment until difference < 12h
        while (awake_getup_diff >= flip_gap_hours) {
      ...
      if (bed < sleep && bed_sleep_diff >= flip_gap_hours) {
        # Loop adjustment until difference < 12h
        while (bed_sleep_diff >= flip_gap_hours) {
