...
- 🔍 **Detects** not auto-fixes — 1,048 records flagged for manual review, 0 silent corrections (field-misentry silent-misrepair bug, 96% in v1.4.0, fixed in v1.4.4+; current benchmark: 0% silent misrepair for SOL/WASO)
...
**Flag statistics (v1.4.5, n=13,990):** 1,048 records flagged (7.5%); 0 AUTO_FIX; 0 silent corrections. Breakdown: 922 TIMESTAMP (window violations), 140 DURATION (order violations), 1 SELF_REPORTED (extreme), 1 redundancy-confirmed worsening. See `VALIDATION_REPORT.md` for full breakdown.
