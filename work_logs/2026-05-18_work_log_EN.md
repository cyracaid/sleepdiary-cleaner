...
| 2 | `possible_decimal_slip` | caffeine 0.3 (pid=6259) | ❌ Flagged only |
| 3 | `suspicious_decimal` | alcohol 1.25 (pid=10009) | ❌ Flagged only |
...
`manual_error_corrections.csv` row for pid=6985 day=8 had the correction text `"Set time_awake equal to time_getup"` incorrectly placed in the `correct_value` column (col 24) instead of `solution_humanidentified` (col 22). This caused the CASE2 handler to miss it, falling through to CASE4 (unprocessable — skipped). Moved to `solution_humanidentified` so CASE2 can now trigger and execute the Awake time align to get up operation.
