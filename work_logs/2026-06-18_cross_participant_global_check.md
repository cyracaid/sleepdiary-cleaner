...
None of these know that **pid 6259 normally SOL=5-30min** — so day 4's SOL=400 (or "04:00" recoded to 4 min via MM:SS) should be interpreted differently than the same value from someone whose baseline is already 150-250 min.
...
Special rule: if personal median SOL < 30 min AND current value > 240 min → auto-flag regardless of deviation score. This directly catches the pid 6259 pattern (normally short SOL, one extreme day that is almost certainly not legitimate).
...
| Check | Verdict | Detail |
|-------|---------|--------|
| Known PIDs (6259, 2835) excluded | ✅ Correct | Manually corrected in Step 6 |
| False positives | ✅ Zero | All 52 rows satisfy triple threshold |
| n_days ≥ 3 | ✅ All pass | Min = 4 (pid 4484) |
...
|-----|-----------|-------|---------------|--------|
| **6374** | **3** | **330-360** | ✅ All human-accepted | Day 8 (330) still flagged by CP |
| 6258 | 3 | 130-150 | ✅ All human-accepted | Already reviewed |
| 3200 | 3 | 120-150 | ✅ All human-accepted | Already reviewed |
| 3131 | 3 | 120-150 | ✅ All human-accepted | Already reviewed |
| **7121** | **2** | **210-315** | ✅ All human-accepted | Day 12 (315) still flagged by CP |
| 6127 | 2 | 120-180 | ✅ All human-accepted | Already reviewed |
| 7772 | 2 | 135-136 | ✅ All human-accepted | Already reviewed |

**PIDs with WASO ≥ 120 on ≥3 days:**
| PID | High Days | Range | All Accepted? |
|-----|-----------|-------|---------------|
| 11062 | 3 | 165-210 | ✅ All human-accepted |

**PIDs with SubjSOL ≥ 120 on ≥2 days:**
| PID | High Days | Range | All Accepted? |
|-----|-----------|-------|---------------|
| 1030 | 3 | 120-180 | ✅ All human-accepted |
| 9696 | 2 | 180-225 | ✅ All human-accepted |
| 10635 | 2 | 120-120 | ✅ All human-accepted |
| 10638 | 2 | 120-180 | ✅ All human-accepted |

**Key finding**: All multi-day extreme-value PIDs were already human-accepted (`confirmed_not_error_do_not_correct`). The CP check correctly defers to human decisions. Cases where a CP flag exists alongside these (e.g., pid 6374 day 8, pid 7121 day 12) are because those specific rows were NOT yet reviewed — CP flags them for new human evaluation.
...
### Synthetic Test (pid 6259 analogue)
```
...
|-----|------|-------------|-------------|---------|
| **1036** | 3 × (day 7,12,13) | time_sleep | 630-660 | Entered sleep time instead of SOL |
| 2835 | 1 × (day 12) | time_sleep | 615 | Entered sleep time instead of SOL |
| 4481 | 1 × (day 7) | time_sleep | 765 | Entered sleep time instead of SOL |
| 6805 | 1 × (day 9) | time_sleep | 725 | Entered sleep time instead of SOL |

**pid 1036** is the most important: 33% of their SOL-non-NA days are field-misentries. The MM:SS parser silently "fixed" these to 10.5 min, making them invisible to both Step 8 and Step 8.5 checks.
...
- Direct reviewer attention needed for pid 1036, 2835, 4481, 6805
...
5. **Review `cross_participant_field_misentries.csv`** → ⏳ Need human evaluation of pid 1036, 2835, 4481, 6805
...
- pid 1036: 3 SOL days match time_sleep (630-660 min) — 33% of their SOL data
- pid 2835 day 12: SOL "10:15" = 615 min
- pid 4481 day 7: SOL "12:45" = 765 min
- pid 6805 day 9: SOL "12:05" = 725 min
...
| ACCEPT_AS_INSOMNIA | 5 | Real insomnia pattern (7415×4, 1872×1), not data-entry errors |
| ACCEPT_AS_INSOMNIA_CP | 3 | CP-overlap records (6374 day8, 7121 day12, 10323 day13) — genuine extreme insomnia days, CP spike is correct but no action needed |
| CORRECT_AWAKE_GETUP_AMPM | 1 | 5310 day14 — awake/getup needs AM/PM conversion (9PM→9AM) |

### Key Findings

- **All 3 CP-overlap rows judged as real insomnia**: pid 6374 SOL=330 (33× baseline median), pid 7121 SOL=315 (31.5×), pid 10323 SE=50% WASO=120 — these are genuine severe insomnia days, not entry errors
- **Only 1 row needs correction**: pid 5310 day 14 — awake/getup from 9PM to 9AM
