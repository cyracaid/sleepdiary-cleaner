...
这些都不知道 **pid 90023 通常 SOL=5-30min**——所以 day 4 的 SOL=400（或 MM:SS 重编码为 4 min）应该与基线本就在 150-250 min 的人区别对待。
...
特殊规则：如果个人中位数 SOL < 30 min 且 当前值 > 240 min → 无条件自动标记。直接捕获 pid 90023 模式（通常短 SOL，某个极端天几乎肯定不合法）。
...
| 检查项 | 结论 | 详情 |
|-------|------|------|
| 已知 PID（90023, 90012）排除 | ✅ 正确 | Step 6 中手动修正 |
| 误报 | ✅ 零 | 所有 52 行满足三重阈值 |
| n_days ≥ 3 | ✅ 全部通过 | 最小 = 4（pid 90018）|
...
| PID | 高天 | 范围 | 全部已接受？| 状态 |
|-----|------|------|------------|------|
| **90024** | **3** | **330-360** | ✅ 全部人工接受 | Day 8 (330) 仍被 CP 标记 |
| 90105 | 3 | 130-150 | ✅ 全部人工接受 | 已审查 |
| 90112 | 3 | 120-150 | ✅ 全部人工接受 | 已审查 |
| 90113 | 3 | 120-150 | ✅ 全部人工接受 | 已审查 |
| **90029** | **2** | **210-315** | ✅ 全部人工接受 | Day 12 (315) 仍被 CP 标记 |
| 90110 | 2 | 120-180 | ✅ 全部人工接受 | 已审查 |
| 90108 | 2 | 135-136 | ✅ 全部人工接受 | 已审查 |

**WASO ≥ 120 且 ≥3 天的 PID：**
| PID | 高天 | 范围 | 全部已接受？|
|-----|------|------|------------|
| 90117 | 3 | 165-210 | ✅ 全部人工接受 |

**SubjSOL ≥ 120 且 ≥2 天的 PID：**
| PID | 高天 | 范围 | 全部已接受？|
|-----|------|------|------------|
| 90119 | 3 | 120-180 | ✅ 全部人工接受 |
| 90040 | 2 | 180-225 | ✅ 全部人工接受 |
| 90104 | 2 | 120-120 | ✅ 全部人工接受 |
| 90118 | 2 | 120-180 | ✅ 全部人工接受 |

**关键发现**：所有多日极端值 PID 都已被人接受（`confirmed_not_error_do_not_correct`）。CP 检查正确遵从人工决定。CP 标记仍存在的情况（如 pid 90024 day 8, pid 90029 day 12）是因为那些特定行尚未被审查——CP 标记它们以待新的人工评估。
...
|-----|------|----------|----------|------|
| **90001** | 3 ×（day 7,12,13）| time_sleep | 630-660 | 将入睡时间填入 SOL 而非分钟数 |
| 90012 | 1 ×（day 12）| time_sleep | 615 | 将入睡时间填入 SOL |
| 90017 | 1 ×（day 7）| time_sleep | 765 | 将入睡时间填入 SOL |
| 90026 | 1 ×（day 9）| time_sleep | 725 | 将入睡时间填入 SOL |

**pid 90001** 最重要：33% 的 SOL 非 NA 天是 field-misentry。MM:SS 解析器静默地"修正"为 10.5 min，使它们对 Step 8 和 Step 8.5 均不可见。
...
- 需要审查者直接关注 pid 90001, 90012, 90017, 90026
...
| ACCEPT_AS_INSOMNIA | 5 | 真实失眠模式（90032×4, 90120×1），非录入错误 |
| ACCEPT_AS_INSOMNIA_CP | 3 | CP 重叠记录（90024 day8, 90029 day12, 90046 day13）— 真实失眠极端日，CP spike 正确但无需操作 |
| CORRECT_AWAKE_GETUP_AMPM | 1 | 90019 day14 — awake/getup 需 AM/PM 转换（9PM→9AM）|

### 关键发现

- **所有 3 条 CP 重叠记录均判定为真实失眠**：pid 90024 SOL=330（33× 基线中位数）、pid 90029 SOL=315（31.5×）、pid 90046 SE=50% WASO=120 — 真实严重失眠日，非录入错误
- **仅 1 行需修正**：pid 90019 day 14 — awake/getup 从 9PM 改为 9AM
...
4. 阈值调优 → ✅ 不需要（零误报）
5. **审查 `cross_participant_field_misentries.csv`** → ⏳ 需要人工评估 pid 90001, 90012, 90017, 90026
