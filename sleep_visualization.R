  ...
  data <- data %>%
    mutate(
      ...
      flag_poor_efficiency = ifelse(!is.na(sleep_efficiency_pct),
        sleep_efficiency_pct < config_get(pipeline_config, "classification.flag_severity.poor_efficiency_threshold_pct", 70), FALSE),
      flag_high_sol = ifelse(!is.na(sol_h),
        sol_h > config_get(pipeline_config, "classification.flag_severity.high_sol_threshold_hours", 1), FALSE),
      flag_high_waso = ifelse(!is.na(waso_h),
        waso_h > config_get(pipeline_config, "classification.flag_severity.high_waso_threshold_hours", 1.5), FALSE),
    ...
    p_p26 <- ggplot(pid_flags, aes(x = pid, y = pct, fill = flag_severity)) +
      ...
      labs(title = "Figure P26: Per-Participant Data Quality (Final Corrected Data)",
           subtitle = paste0("Each bar = one participant (N=", length(unique(clean_df$pid)),
                             ...
                             config_get(pipeline_config, "classification.flag_severity.poor_efficiency_threshold_pct", 70),
                             "%, SOL>", config_get(pipeline_config, "classification.flag_severity.high_sol_threshold_hours", 1),
                             "h, WASO>", config_get(pipeline_config, "classification.flag_severity.high_waso_threshold_hours", 1.5), "h}."),
