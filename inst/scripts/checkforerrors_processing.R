...
.s_col <- function(key, fallback) cfg_get(paste0("column_mapping.substance.", key), fallback, cfg = pipeline_config)
...
can_col <- .s_col("cannabis", "cannabis_amount_pm_doses")
  ...
  raw_csv <- cfg_get("data.files.extra", "your_extra_data.csv", cfg = pipeline_config)
...
# Load raw CSV values for text-preserving detection
  raw_csv_fname <- cfg_get("data.files.extra", "your_extra_data.csv", cfg = pipeline_config)
raw_csv_data <- NULL
...
if (all(required_metrics %in% names(data))) {
  ...
  sol_excessive <- cfg_get("classification.metric_validation.sol.excessive_minutes", 120, cfg = pipeline_config)
  ...
  se_insane <- cfg_get("classification.metric_validation.se.insane_negative_percent", -1000, cfg = pipeline_config)
  se_min <- cfg_get("classification.metric_validation.se.min_valid_percent", 0, cfg = pipeline_config)
  se_max <- cfg_get("classification.metric_validation.se.max_valid_percent", 100, cfg = pipeline_config)
  ...
  ratio_min <- cfg_get("classification.metric_validation.tst_tib_ratio.min_ratio", 0.5, cfg = pipeline_config)
  ratio_max <- cfg_get("classification.metric_validation.tst_tib_ratio.max_ratio", 1.0, cfg = pipeline_config)
...
accept_path <- cfg_get("data.files.manual_metric_accept", "manual_metric_review_acceptances.csv", cfg = pipeline_config)
...
if (review_count > 0) {
  ...
  cat(sprintf("    SOL excessive (>&nbsp;%dmin):         %d\n", 
              cfg_get("classification.metric_validation.sol.excessive_minutes", 120, cfg = pipeline_config), sr_sol_excessive))
