...
# ============================================================================
# Figure 12 MERGED INTO STEP FLAG LEDGER (per VISUALIZATION_TREE_PROPOSAL.md §2 1.1)
# ============================================================================
# The former standalone "Figure 12: Pipeline Correction Progress" three-panel
# table (12_Pipeline_Correction_Progress.png) is removed. Its question --
# per-step counts and convergence -- is answered by the Step Flag Ledger
# (A1_Step_Flag_Ledger.png, generated by R/figure12_step_flag_table.R via
# generate_appendix_ledger() at the end of this script), which shows the same
# per-step counts against shared final standards, plus corrected/suppressed
# columns the old grid.arrange table lacked. The old figure also had a
# confirmed dead-space bug (grid.arrange fixed absolute row heights vs the
# 9-inch canvas, ~3.9in blank at bottom); merging removes it outright.
#
# Per-step metrics for reference (previously panels 1-3 of Fig 12) are still
...
if(checkforerrors_exists && nrow(checkforerrors_processed) > 0) {
  ...
  # --------------------------------------------------------------------------
  if (cfg_get("visualization.gap_analysis.enabled", TRUE, cfg = pipeline_config)) {
  ...
  # --------------------------------------------------------------------------
  if (cfg_get("visualization.detection_outcomes.enabled", TRUE, cfg = pipeline_config)) {
  ...
  # --------------------------------------------------------------------------
  if (cfg_get("visualization.threshold_noise.enabled", TRUE, cfg = pipeline_config)) {
...
if (exists("checkforerrors_summary") && is.list(checkforerrors_summary) && 
  ...
  # --------------------------------------------------------------------------
  # Figure 19 FOLDED INTO FIGURE 13 (per VISUALIZATION_TREE_PROPOSAL.md §2 1.2)
  # --------------------------------------------------------------------------
  # The former standalone "Figure 19: Final Data Quality Status" bar chart is
  # dropped as a separate file. Its content -- the CLEAN vs DURATION_ISSUE vs
  # TIMESTAMP_ISSUE vs SELF_REPORTED_FLAG split -- is exactly what Figure 13's
  # flag_dist_df table (flag_tab above) already renders, coarser but identical
  # in source (checkforerrors_summary$review_summary). Keeping both was
  # redundant; the fold removes the duplicate, not the information.
  #
  # NEEDS_REVIEW detail (72 records, retained for reference):
  ...
  #     SOL excessive ( > 120 min )          61
  #     SOL zero      ( == 0 min )            2
  #     SOL lt15      ( < 15 min )            3
  #     SE anomalies  ( efficiency < 0% )     6
  #                          Total:          72
  #   These require case-by-case human judgment (genuine insomnia vs data error).
  # --------------------------------------------------------------------------
...
cat("  Figure 12: MERGED into Step Flag Ledger (A1) — per-step flag counts table\n\n")
...
cat("  Figure 19: FOLDED into Figure 13 (final-status split shown in Fig 13's flag table)\n")
